...
export class AuthService {
    ...
    async signUp(createUserDto: CreateUserDto) {
        ...
        // Find the Role by name from the roleName
        ...
        const { id } = await this.prismaService.user.create({
            data: {
                ...
                role: { connect: { id: role$.id } }, // Connect the role,
            },
        })

        // Sign jwt: Generate access token and return it
        const accessToken = this.jwtService.sign({ sub: id, username, role })
        if (!accessToken) {
    ...
    async signIn(userDto: UserDto) {
        ...
        let roleName = await this.redisService.getRoleName(foundUser.roleId)
        if (!roleName) {
            roleName = 'User'
        }
